...
def main():
    st.title("Mushrooms Classification Web App")
    st.sidebar.title("Mushrooms Classification Web App")
    st.markdown("Are your mushrooms edible or poisonous? 🍄  ")
    st.sidebar.markdown("Are your mushrooms edible or poisonous? 🍄")

    @st.cache(persist=True)
    def load_data():
        '''put hte dataset path here'''
        path="D:\github\github_projects\Machine_learning projects\streamlit-ml\mushrooms.csv"
    ...
    classifier = st.sidebar.selectbox("Classifier", ("Support Vector Machine (SVM)", "Logistic Regression", "Random Forest", "K-nearest-neighbors", "Naive Bayes","Decision_tree"))
    ...
    if classifier == 'K-nearest-neighbors':
        st.sidebar.subheader("Model Hyperparameters")
        n_neighbors = st.sidebar.number_input("The number of neighbors", 3, 50, step=1, key='n_neighbors')
        metrics = st.sidebar.multiselect("What metrics to plot?", ('Confusion Matrix', 'ROC Curve', 'Precision-Recall Curve'))
    ...
    if classifier == 'Decision_tree':
        ...
        max_depth = st.sidebar.number_input("The maximum depth of the tree", 1, 20, step=1, key='criterion')
        metrics = st.sidebar.multiselect("What metrics to plot?", ('Confusion Matrix', 'ROC Curve', 'Precision-Recall Curve'))

        if st.sidebar.button("Classify", key='classify'):
            st.subheader("Decision_tree Results")
